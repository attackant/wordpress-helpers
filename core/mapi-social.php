...
function mapi_google_plus_one( $args ) {
	...
	?>
    <!-- Google +1 button -->
    <div class="g-plusone mcms-social" data-size="<?php echo $args['width']; ?>"
         data-annotation="<?php echo $args['annotation']; ?>" data-href="<?php echo $args['href']; ?>"
...
function mapi_social_links( $networks = array(), $share_or_follow = 'share', $echo = true, $js = false ) {
	...
	if ( $echo == true ) {
		...
		foreach ( $networks as $network ) {
			if ( $js == true ) {
				mapi_social_link_js( array(
					'network' => $network,
					'share'   => $share,
					'class'   => $mapi_social_links_class
				) );
			} else {
				mapi_social_link( array(
					'network' => $network,
					'share'   => $share,
					'class'   => $mapi_social_links_class
				) );
		...
		if ( $args['echo'] ) {
			...
			foreach ( $tweets as $tweet ) : ?>
                <li>
                    ...
                    <span class="mapi-tweet-meta">Posted to <a class="mapi-tweet-link"
                                                               href='https://twitter.com/<?php echo $args['screen_name']; ?>/status/<?php echo $tweet->id; ?>'
                                                               title='<?php echo 'Posted ' . $tweet->created_at; ?>'>Twitter</a> on <?php echo $tweet->created_at; ?>
												  by <a href="https://twitter.com/<?php echo $args['screen_name']; ?>"
                                                        title="Connect with <?php bloginfo( 'name' ); ?> on Twitter"
                                                        target="_blank"><?php echo $args['screen_name']; ?></a></span>
                </li>
			<?php
			endforeach;
			echo '</ul>';
