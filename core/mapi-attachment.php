...
function mapi_featured_img( $args = array() ) {
	...
	$defaults = array(
		...
		'q'     => apply_filters( 'mapi_thumb_q', 90 ),
		// quality 0-100
		'a'     => apply_filters( 'mapi_thumb_a', 'c' ),
		// crop alignment c, t, l, r, b, tl, tr, bl, br	(c = center, t = top, b = bottom, r = right, l = left)
		'zc'    => apply_filters( 'mapi_thumb_zc', 1 ),
		// zoom/crop
		'f'     => apply_filters( 'mapi_thumb_f', null ),
		's'     => apply_filters( 'mapi_thumb_s', 0 ),
		// sharpen 1 or 0
		'cc'    => apply_filters( 'mapi_thumb_cc', null ),
		// canvas color ffffff
		'ct'    => apply_filters( 'mapi_thumb_ct', 1 ),
		// canvas transparency (overrides cc) 1 or 0
	);
	...
	if ( has_post_thumbnail( $args['id'] ) ) {
		...
		if ( $args['echo'] === true ) {
			...
			?>
            <img alt="<?php echo $args['alt']; ?>" <?php if ( $args['title'] ) : echo 'title="' . $args['title'] . '"'; endif; ?>
                 src="<?php echo $args['img']; ?>"/>
			<?php
...
function mapi_thumb_array( $args ) {
	...
	$defaults = array(
		...
		'h'   => apply_filters( 'mapi_thumb_h', null ),
		...
		// 		3	Resize proportionally adjusting size of scaled image so there are no borders gaps)
		'f'   => apply_filters( 'mapi_thumb_f', null ),
		// filters (can be combined) f=FILTER_ID,FILTER_PARAM,FILTER_PARAM|FILTER_ID,FILTER_PARAM
		//		1 = Negate - Invert colours
		// 		2 = Grayscale - turn the image into shades of grey
...
function mapi_random_img( $args ) {
	...
	if ( $args['echo'] ) {
		...
		?>
        <img class="mapi-random-img" src="<?php echo $src ?>" width="<?php echo $args['width'] ?>"
             height="<?php echo $args['height'] ?>" alt="<?php echo $args['alt'] ?>"
             title="<?php echo $args['alt'] ?>"/>
		<?php
