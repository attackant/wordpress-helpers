...
if ( ! class_exists( 'mindshare_options_framework' ) ) :
	class mindshare_options_framework {
		...
		public function AddMenuTopPage() {
			...
			$page       = add_submenu_page( $top_id, $this->args['page_title'], $this->args['menu_title'], $this->args['capability'], $this->args['id'], array(
				$this,
				'DisplayPage'
			) );
			if ( $page ) {
		...
		public function AddMenuSubPage() {
			...
			$this->args = array_merge( $default, $this->args );
			$page       = add_submenu_page( $this->Top_Slug, $this->args['page_title'], $this->args['menu_title'], $this->args['capability'], $this->args['id'], array(
				$this,
				'DisplayPage'
			) );
			if ( $page ) {
		...
		public function DisplayPage() {
                ...
                <div class="updated">
                    <p>All options have been removed from the database.
						<?php if ( $this->project_path == 'PLUGIN' ) : ?>
							...
							?>
                            To complete the uninstall <a
                                    href="<?php echo $deactivate_url; ?>">deactivate <?php echo $this->project_name ?>
                                .</a>
						<?php elseif ( $this->project_path == 'THEME' ) : ?>
                            To complete the uninstall <a href="themes.php">deactivate <?php echo $this->project_name ?>
                                .</a>
						<?php endif; ?>
                    </p>
                </div>
			...
			if ( isset( $_POST['mindshare_framework_reset'] ) ) {
				...
				?>
                <div class="updated">
                    <p>All options have been restored to their default values.</p>
                </div>
				<?php
			}
            ...
            <div class="footer_wrap">
                <p class="submit">
                    <input type="submit" name="Submit"
                           class="<?php echo apply_filters( 'mindshare_options_framework_submit_class', 'button-primary' ); ?>"
                           value="<?php echo esc_attr( __( 'Save Changes' ) ); ?>"/>
                    <input type="hidden" name="action" value="save"/>
					<?php if ( $this->show_reset_button == true ) : ?>
                        <input class="button-secondary" type="button" value="Restore Defaults"
                               onclick="document.getElementById('mindshare-framework-reset').style.display = 'block';document.getElementById('mindshare-framework-uninst').style.display = 'none';"/>
					<?php endif; ?>
					<?php if ( $this->show_uninstall_button == true ) : ?>
                        <input class="button-secondary" type="button" value="Uninstall"
                               onclick="document.getElementById('mindshare-framework-uninst').style.display = 'block';document.getElementById('mindshare-framework-reset').style.display = 'none';"/>
					<?php endif; ?>
                </p>
            </div>
            </div>
            ...
            <div id="mindshare-framework-uninst" style="display:none; clear: both;">
                <form method="post" action="">
                    ...
                    <label style="font-weight:normal;">Do you wish to <strong>completely unistall</strong>
                        the <?php echo $this->project_name ?> plugin?</label>
                    <input class="button-secondary" type="button" name="cancel" value="Cancel"
                           onclick="document.getElementById('mindshare-framework-uninst').style.display = 'none';"
            ...
            <div id="mindshare-framework-reset" style="display:none; clear: both;">
                <form method="post" action="">
                    ...
                    <label style="font-weight:normal;">Do you wish to <strong>completely reset</strong> the default
                        options for <?php echo $this->project_name ?>?</label>
                    <input class="button-secondary" type="button" name="cancel" value="Cancel"
                           onclick="document.getElementById('mindshare-framework-reset').style.display='none';"
                           style="margin-left:20px"/>
                    <input class="button-primary" type="submit" name="mindshare_framework_reset"
		...
		public static function validate( $field, $new_value ) {
			if ( ! is_array( $field ) ) {
				...
				if ( isset( $field['validation_function'] ) ) {
					...
					return $new_value; // disabled auto validation, needs more testing
				}
			}
		}
